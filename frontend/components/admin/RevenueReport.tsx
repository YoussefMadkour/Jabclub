  ...
  return (
    <div className="space-y-6">
      <div className="bg-[#16161a] border border-[#26262B] rounded-lg shadow p-6">
        <h2 className="text-2xl font-bold mb-6 text-gray-300">Revenue Report</h2>
              ...
              <div>
                ...
                <div className="bg-[#121214] rounded-lg p-4">
                  <div className="space-y-3">
                    ...
                    <div className="flex justify-between items-center">
                      <span className="text-sm text-gray-400">Pending Revenue</span>
                      <span className="text-lg font-bold text-yellow-600">
                        EGP {reportData.summary.pendingRevenue}
                      </span>
                    </div>
                    <div className="border-t border-[#26262B] pt-3 flex justify-between items-center">
                      <span className="text-sm font-medium text-gray-300">Total Potential</span>
                  ...
                  <div className="space-y-2">
                    {Object.entries(reportData.pendingPaymentsSummary.packages).map(([packageName, data]) => (
                      <div key={packageName} className="flex justify-between items-center">
                        <span className="text-sm text-gray-400">{packageName}</span>
                        <div className="text-right">
                          <div className="text-sm font-semibold">{data.count} pending</div>
                          <div className="text-xs text-gray-400">EGP {typeof data.value === 'string' ? parseFloat(data.value).toFixed(2) : data.value.toFixed(2)}</div>
                        </div>
                      </div>
                    ))}
                  </div>
                    ...
                    <tbody className="bg-[#16161a] border border-[#26262B] divide-y divide-[#26262B]">
                      {reportData.recentPayments.map((payment) => (
                        <tr key={payment.id}>
                          ...
                          <td className="px-4 py-3 text-sm text-gray-400">{payment.email}</td>
