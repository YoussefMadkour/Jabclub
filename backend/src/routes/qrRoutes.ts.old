...
router.post('/generate/:bookingId', async (req: any, res) => {
  try {
    ...
    // Check if the class is within 2 hours (prevent early QR generation)
    const now = new Date();
...
router.post('/validate', async (req: any, res) => {
  try {
    ...
    // Verify the booking matches the QR data
    ...
    // Check if the coach is assigned to this class
